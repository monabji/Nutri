...
describe('automatic route resolution', () => {
  ...
  it('uses a source-reported manufacturing place, fixed Katpadi destination, driving route, and route weather', async () => {
    vi.stubGlobal('fetch', vi.fn((url: string) => {
      ...
      if (url.includes('router.project-osrm.org')) return Promise.resolve(new Response(JSON.stringify({ code: 'Ok', routes: [{ distance: 575000, duration: 37800, geometry: { coordinates: [[78.4867, 17.385], [79.1368, 12.9818]] } }] })))
    ...
    expect(result).toMatchObject({ status: 'available', value: { distanceKm: 575, durationHours: 10.5, routingKind: 'driving', destination: { label: 'Katpadi, Vellore' }, weather: { averageTemperatureC: 32 } } })
  })
