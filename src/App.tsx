import { ArrowDownRight, ArrowLeft, ArrowUpRight, Camera, Leaf, Menu, ScanLine, Search, X } from 'lucide-react'
...
function Dashboard({ onBack }: { onBack: () => void }) {
  ...
  return <main className="dashboard-shell">
    ...
    <section className="dashboard-hero" aria-labelledby="dashboard-title"><p className="hero-index">Product explorer / phases 3–4</p><h1 id="dashboard-title">Start with the record. Then make the journey visible.</h1><p>Look up a packaged food by barcode, then explore transparent transport scenarios without confusing estimates with source facts.</p></section>
<section className="lookup-panel" aria-labelledby="lookup-title"><div><p className="eyebrow">Live barcode lookup</p><h2 id="lookup-title">Reveal the record.</h2></div><form onSubmit={submit} noValidate><label htmlFor="barcode">Product barcode</label><div className="lookup-control"><input id="barcode" inputMode="numeric" autoComplete="off" value={barcodeInput} onChange={(event) => { setBarcodeInput(event.target.value); setValidationMessage('') }} placeholder="e.g. 8906009532363" aria-describedby="barcode-help barcode-error" /><button className="camera-scan-button" type="button" onClick={() => setScannerOpen(true)} aria-label="Scan barcode with camera"><Camera size={18} /><span>Camera</span></button><button className="primary-button" type="submit" disabled={lookup.status === 'loading'}>{lookup.status === 'loading' ? 'Looking up…' : <>Look up <Search size={16} /></>}</button></div><p id="barcode-help" className="field-help">Supports 8–14 digit product codes; spaces and hyphens are accepted. On mobile, use Camera to scan.</p>{validationMessage ? <p id="barcode-error" className="field-error" role="alert">{validationMessage}</p> : null}</form><div className="preset-row"><span>Try a documented barcode</span>{presets.map((preset) => <button type="button" key={preset.barcode} onClick={() => choosePreset(preset.barcode)}>{preset.label}</button>)}</div></section>
    {lookup.status === 'success' ? <ProductDashboard product={lookup.product} missingFields={lookup.missingFields} scenario={scenario} route={route} routeLoading={routeLoading} onScenarioChange={handleScenarioChange} onResolveRoute={handleResolveRoute} onRetryAutomaticRoute={() => void applyAutomaticRoute(lookup.product)} /> : <section className="dashboard-empty"><DataState type={lookup.status} message={lookup.status === 'error' ? lookup.message : undefined} onRetry={() => void lookupBarcode(barcodeInput)} /></section>}
    {scannerOpen ? <BarcodeCameraScanner onDetected={handleDetected} onClose={() => setScannerOpen(false)} /> : null}<Footer />
  </main>
}

type ProductDashboardProps = { product: ProductFacts; missingFields: string[]; scenario: ScenarioInput; route: Availability<ResolvedRoute>; routeLoading: boolean; onScenarioChange: (patch: Partial<ScenarioInput>) => void; onResolveRoute: () => void; onRetryAutomaticRoute: () => void }
function ProductDashboard({ product, missingFields, scenario, route, routeLoading, onScenarioChange, onResolveRoute, onRetryAutomaticRoute }: ProductDashboardProps) {
  const partial = missingFields.length > 0
  return <section className="product-dashboard" aria-labelledby="product-title">
    <div className="product-summary"><div><p className="eyebrow">Live product record</p><h2 id="product-title">{product.name || 'Unnamed product'}</h2><p className="product-brand">{product.brands?.join(' · ') || 'Brand not reported by Open Food Facts.'}</p></div><span className={partial ? 'live-badge live-badge--partial' : 'live-badge'}>{partial ? 'Partial source record' : 'Source-reported facts'}</span></div>
    <div className="metric-grid"><MetricCard label="NOVA group" value={product.novaGroup ? `Group ${product.novaGroup}` : undefined} /><MetricCard label="Eco-Score" value={product.ecoScore} /><MetricCard label="Quantity" value={product.quantityGrams ? `${product.quantityGrams} g` : undefined} /><MetricCard label="Additives" value={product.additives?.length} /></div>
    <div className="facts-layout"><div className="fact-stack"><article className="fact-card"><p className="eyebrow">Ingredients</p><h3>What is listed</h3><p>{product.ingredientsText || 'Not reported by Open Food Facts.'}</p></article><article className="fact-card"><p className="eyebrow">Origins & packaging</p><div className="fact-pairs"><div><h3>Manufacturing place</h3><p>{product.manufacturingPlaces?.join(', ') || 'Not reported by Open Food Facts.'}</p></div><div><h3>Origins</h3><p>{product.origins?.join(', ') || 'Not reported by Open Food Facts.'}</p></div><div><h3>Countries</h3><p>{product.countries?.join(', ') || 'Not reported by Open Food Facts.'}</p></div><div><h3>Packaging</h3><p>{product.packaging?.map((item) => item.label).join(', ') || 'Not reported by Open Food Facts.'}</p></div></div></article><article className="fact-card"><p className="eyebrow">Nutrition per 100 g</p><h3>Reported nutrients</h3>{Object.keys(product.nutrients).length ? <dl className="nutrient-list">{Object.entries(product.nutrients).map(([name, nutrient]) => <div key={name}><dt>{name}</dt><dd>{nutrient.value} {nutrient.unit}</dd></div>)}</dl> : <p>Not reported by Open Food Facts.</p>}</article></div><SourcePanel product={product} missingFields={missingFields} /></div>
