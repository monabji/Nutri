...
const productPayload = {
  ...
  product: {
    ...
    nutriments: { 'energy-kcal_100g': 380, proteins_100g: 20 },
  },
}
...
describe('sork.', () => {
  ...
  it('opens the dashboard from the landing page', () => {
    ...
    expect(screen.getByRole('heading', { name: /start with what the product record/i })).toBeInTheDocument()
  })

  it('validates barcodes before requesting data', () => {
    render(<App />)
    fireEvent.click(screen.getAllByRole('button', { name: /explore a product/i })[0])
    fireEvent.change(screen.getByLabelText(/ean-13 barcode/i), { target: { value: '1234' } })
    fireEvent.click(screen.getByRole('button', { name: /^look up$/i }))
    expect(screen.getByRole('alert')).toHaveTextContent('Enter a valid 13-digit EAN barcode.')
  })
  ...
  it('shows normalized, source-attributed facts from a successful lookup', async () => {
    ...
    render(<App />)
    fireEvent.click(screen.getAllByRole('button', { name: /explore a product/i })[0])
    fireEvent.change(screen.getByLabelText(/ean-13 barcode/i), { target: { value: '8906 0095-32363' } })
    fireEvent.click(screen.getByRole('button', { name: /^look up$/i }))
